...
("41", "Design", "Curso de Design avançado", "30", "25", "2015"),
("42", "Python", "Curso de Python avançado", "40", "35", "2020"),
("43", "Excel", "Curso de Excel profissional", "20", "18", "2018");
...
select distinct carga from cursos order by carga; -- SÓ IRÁ CONSIDERAR UMA OCORRÊCIA DE CADA VALOR DENTRO DO REGISTRO
...
/*PARTE 3*/
/*O 'GROUP BY' é usada para agrupar linhas que possuem os mesmos valores em uma ou mais colunas.*/
select carga from cursos group by carga;

select totalaulas, count(*) from cursos group by totalaulas;

select carga, count(*) from cursos where totalaulas = 30
order by carga;
